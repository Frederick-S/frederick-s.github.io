...
* 没有严格按照 `Figure 2` 的要求重置 `election timeout`。重置 `election timeout` 仅限于几种情况，第一，收到当前任期的主节点的 `AppendEntries` 请求（如果 `AppendEntries` 中的任期比自身的任期小，则忽略该请求）；第二，开启一轮新的选主，这个属于 `election timeout` 到期，需要重置一个新的随机值；第三，在某轮选举中，将票投给了某个候选节点。
