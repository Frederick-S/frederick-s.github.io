...
* 确保周期性的检查 `commitIndex > lastApplied` 或者在 `commitIndex` 更新后检查（在 `matchIndex` 更新后）。这里并没有太明白这个问题，作者举了个例子，假设现在主节点发出了 `AppendEntries` 请求并同时检查 `commitIndex`，如果这条日志被过半数的节点复制了，那么需要等到主节点再添加一条新日志后才能应用上一条日志到状态机，不过根据这个例子也没有看出两者的关联。因为如果采用单线程顺序应用日志到状态机的方式，如果 `commitIndex > lastApplied` 不满足，线程可以先睡眠然后再尝试。
