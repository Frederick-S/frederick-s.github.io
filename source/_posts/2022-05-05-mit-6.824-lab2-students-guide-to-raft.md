...
* `AppendEntries` 中的第五步的 `min` 是有必要的，而且是和新日志中的最后一条日志的索引相比较，而不是当前日志的最后一条索引。首先来看 `leaderCommit < index of last new entry` 的情况，假设 `S1` 为主节点，日志为 `[1, 2, 3, 4, 5]`，`leaderCommit` 为3（索引按从1开始算），`S2` 为从节点，日志为 `[1, 2]`，那么 `S1` 需要将 `[3, 4, 5]` 发给 `S2`，`S2` 收到后 `index of last new entry` 是5，但是 `commitIndex` 只能更新到 `3`，也就是 `leaderCommit`，因为 `[4, 5]` 还没有提交。
