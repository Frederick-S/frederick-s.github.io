...
## 自定义方法
标准方法提供了对资源的基础操作功能，它们的职责都较为单一，基本上对应了基础的 `CRUD` 操作。不过，并不是所有对资源的操作都能或者适合抽象为 `CRUD` 操作，这也是对于 `RESTful` 服务经常争论的地方。因此，自定义方法就应运而生。

不过，对于 `API` 的设计者而言应当尽可能的首选使用标准方法，因为标准方法有着更为统一的语义，对开发者而言更为简单易懂。

自定义方法可以应用于资源，资源集合或者服务。它可能接收任意类型的输入和返回任意类型的输出，并且支持流式的请求和响应。
