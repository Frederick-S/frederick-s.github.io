...
* `Leveled Compaction`：借鉴自 `LevelDB` 并加以改进。每一层可容纳的文件大小呈指数级放大。系统会积极的触发压缩以确保每层的文件大小不会超过指定阈值
* `Tiered Compaction`：在 `RocksDB` 也被称为 `Universal Compactioin`，与 `Apache Cassandra` 或 `HBase` 采取的压缩模式类似。当 `Level-0` 层文件的个数或者非 `Level-0` 层的个数超过指定的阈值，又或者整个数据库的大小和最深层文件大小之比超过指定的阈值时，则会触发压缩并合并多个 `SSTable`。有别于 `Leveled Compaction`，`Tiered Compaction` 是惰性压缩，实际的压缩会推迟到读性能或者空间效率发生衰减时进行，从而能够一次性压缩更多的数据
* `FIFO Compaction`：当数据库大小触及到指定阈值时，丢弃最老的 `SSTable`，且只进行轻量级的压缩。适合于基于内存的缓存应用

`RocksDB` 的读写性能在不同的压缩策略下有着不同的表现，应用开发者需要结合自身的工作负载选择合适的压缩策略。
