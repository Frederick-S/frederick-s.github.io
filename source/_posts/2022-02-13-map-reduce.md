...
如果要开启这个功能，用户需要编写一个命名计数器，然后在 `map` 或 `reduce` 函数中在需要的时候对计数器自增，例如：
...
## 性能
这一节主要描述 `MapReduce` 在 `Google` 内部环境下运行的性能情况，这里不再赘述。简单举例来说，在1800台机器上执行一个 `10T` 数据量的分布式 `grep` 耗时约150秒。
