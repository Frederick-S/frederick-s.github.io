...
当然 `M` 和 `R` 的数量也是存在上限的，对于 `master` 节点来说，它需要维护 `M` 个 `map` 任务和 `R` 个 `reduce` 任务，时间复杂度是 `O(M + R)`，另一方面每个 `map` 任务会产出 `R` 份中间结果数据，对应 `R` 个 `reduce` 任务，所以 `master` 节点也需要 `O(M * R)` 的空间复杂度来维护这样的对应关系（`Google` 实际实现时，每个 `map/reduce` 对约占据 `1 byte` 内存）。
