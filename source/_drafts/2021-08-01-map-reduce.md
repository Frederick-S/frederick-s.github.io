...
如果每一个数据加工任务都需要独立去解决上述的问题，一方面会使得原本简单的代码逻辑变得庞大、复杂和难以维护，另一方面也是在重复工作。因此，`Google` 的工程师们抽象出了 `MapReduce` 框架，使得应用开发人员只需关注计算逻辑实现而无需关心底层运行细节，转而由框架层统一处理并行、容错、数据分发和负载均衡等问题。
