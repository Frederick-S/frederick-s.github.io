...
4. 暂存在内存中的中间结果键值对会周期性的写入到本地磁盘中，注意如前文提到这些磁盘上的数据由 `Google` 内部的分布式文件存储系统所管理，所以当前工作节点产生的中间结果键值对并不是直接写入到当前机器的磁盘上，而是会由某个分片函数将这些数据分发到文件存储系统下的 `R` 个区。同时，这些数据写入后的地址会回传给 `Master` 节点，`Master` 节点会在后续的流程中将对应数据的地址发送给 `Reduce` 节点。
